...
export function useKpResource(endpoint: string) {
  ...
  const fetchRows = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);
      const res = await fetch(`/api/v1/k-platform/${endpoint}`);
      if (!res.ok) {
  ...
  const updateRow = async (id: string, data: Record<string, unknown>) => {
    setSubmitting(true);
    try {
      const res = await fetch(`/api/v1/k-platform/${endpoint}/${id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(data),
      });
  ...
  const deleteRow = async (id: string) => {
    const res = await fetch(`/api/v1/k-platform/${endpoint}/${id}`, { method: "DELETE" });
