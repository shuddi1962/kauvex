...
export async function requireVendor(request: Request) {
  ...
  const adminDb = getAdminClient();
  const { data: profile } = await adminDb.from("profiles").select("role, vendor_id").eq("id", user!.id).maybeSingle();

  let vendorId = profile?.vendor_id || null;

  // If profile doesn't have vendor_id, try looking up vendor by user_id
  if (!vendorId) {
    const { data: vendorByUser } = await adminDb
      .from("vendors")
      .select("id")
      .eq("user_id", user!.id)
      .maybeSingle();
  ...
  const { data: vendor } = await adminDb.from("vendors").select("*").eq("id", vendorId).maybeSingle();
