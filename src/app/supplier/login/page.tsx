...
const SUPPLIER_ROLES = ["supplier", "vendor"];
...
export default function SupplierLoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!user || !SUPPLIER_ROLES.includes(user.role)) {
        setError("Access denied. This account does not have supplier privileges.");
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#0A1628] to-[#1a2a4a] flex items-center justify-center px-6 py-12 relative overflow-hidden">
      ...
      <div className="absolute inset-0 opacity-10">
        ...
        <div className="absolute top-1/2 right-1/3 w-48 h-48 rounded-full bg-orange-500/10" />
      </div>
      ...
      <div className="w-full max-w-md relative z-10">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="font-syne font-800 text-2xl text-white mb-1">Supplier Portal</h1>
          <p className="text-orange-300 text-sm">Supply products to KAUVEX customers</p>
        </div>

        <div className="bg-white rounded-2xl shadow-2xl border border-border p-8">
          <div className="mb-6">
            <h2 className="font-syne font-700 text-xl text-text-1 mb-1">Supplier Sign In</h2>
            <p className="text-text-3 text-xs">Enter your supplier credentials to access your dashboard</p>
          </div>
          ...
          <div className="mt-6 pt-4 border-t border-border space-y-3">
            <p className="text-center text-sm text-text-3">
              Want to supply on KAUVEX?{" "}
              <Link href="/supplier/register" className="text-[#FF6B00] font-semibold hover:underline">
                Apply Now
              </Link>
            </p>
