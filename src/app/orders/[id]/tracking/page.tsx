...
import { Package, Truck, MapPin, CheckCircle2, XCircle, Clock, ChevronRight, Star, Phone, MessageCircle, Shield, Navigation } from "lucide-react";
...
const timelineSteps: TrackingEvent[] = [
...
export default function OrderTrackingPage() {
  ...
  const getStatusDisplay = () => {
    switch (detail.status) {
      ...
      case "packed": return { icon: Package, text: "Packed and ready", color: "text-orange" };
      case "picked_up": case "in_transit": return { icon: Truck, text: "On its way to you", color: "text-orange" };
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-5">
          ...
          {detail.status === "out_for_delivery" && (
            <div className="mt-4 bg-orange-50 border border-orange-200 rounded-lg p-3">
              ...
              <p className="text-2xl font-bold text-orange tracking-widest">{detail.deliveryPin}</p>
              ...
              <div key={step.status} className="flex gap-3">
                <div className="flex flex-col items-center">
                  <div className={`w-5 h-5 rounded-full flex items-center justify-center ${
                    step.completed ? "bg-green-500" : step.active ? "bg-orange border-2 border-orange-200" : "bg-gray-200"
                  }`}>
                ...
                <div className={`pb-6 ${step.active ? "opacity-100" : step.completed ? "opacity-70" : "opacity-40"}`}>
                  <p className={`text-sm font-medium ${step.active ? "text-orange" : step.completed ? "text-green-700" : "text-gray-500"}`}>
                    {step.label}
                  </p>
        ...
        {detail.status === "out_for_delivery" && (
          <div className="bg-white rounded-xl border border-gray-200 p-5">
            <h3 className="font-semibold text-sm text-[#0A1628] mb-3 flex items-center gap-2">
              <Navigation size={14} className="text-orange" /> Live Tracking
            </h3>
            ...
            <div className="flex gap-2 mb-3">
              {[1, 2, 3, 4, 5].map((star) => (
                <button key={star} onClick={() => { setRating(star); setRated(true); }} className={`w-10 h-10 rounded-lg flex items-center justify-center ${
                  star <= rating ? "bg-orange text-white" : "bg-gray-100 text-gray-400"
                }`}>
