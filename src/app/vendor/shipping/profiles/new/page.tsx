...
export default function NewShippingProfilePage() {
  ...
  const [handoffMethod, setHandoffMethod] = useState("auto");
  ...
  const handleSubmit = async () => {
    ...
    setTimeout(() => {
      setSubmitting(false);
      showToast("Shipping profile created successfully", "success");
  ...
  return (
    <VendorShell title="Create Shipping Profile" subtitle="Set up a new shipping rate profile">
      {toast.show && (
      ...
      <div className="max-w-4xl mx-auto">
          ...
          {step === 1 && (
            <div className="space-y-5">
              <div>
                ...
                <p className="text-xs text-gray-400">Name your profile and set it as default if needed.</p>
              </div>
              <div>
        ...
        <div className="mt-4 bg-white rounded-xl border border-gray-200 p-4">
          <h4 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">Profile Summary</h4>
          <div className="grid grid-cols-3 gap-4 text-xs">
            <div>
              <span className="text-gray-400">Name:</span>{" "}
              <span className="font-medium text-gray-700">{profileName || "Not set"}</span>
            </div>
            <div>
              <span className="text-gray-400">Carriers:</span>{" "}
              <span className="font-medium text-gray-700">{selectedCarriers.length}</span>
            </div>
            <div>
              <span className="text-gray-400">Pricing:</span>{" "}
              <span className="font-medium text-gray-700 capitalize">{pricingType.replace(/_/g, " ")}</span>
            </div>
            <div>
              <span className="text-gray-400">Countries:</span>{" "}
              <span className="font-medium text-gray-700">{selectedCountries.length}</span>
            </div>
            <div>
              <span className="text-gray-400">Products:</span>{" "}
              <span className="font-medium text-gray-700">{selectedProducts.length}</span>
            </div>
            <div>
              <span className="text-gray-400">Default:</span>{" "}
              <span className={`font-medium ${isDefault ? "text-amber-600" : "text-gray-400"}`}>{isDefault ? "Yes" : "No"}</span>
            </div>
          </div>
        </div>
      </div>
